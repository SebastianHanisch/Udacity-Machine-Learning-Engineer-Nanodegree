...
class Simulator(object):
    ...
    def render(self, trial, testing=False):
        ...
        for intersection, traffic_light in self.env.intersections.items():
            ...
            self.pygame.draw.circle(self.screen, self.road_color, (int(intersection[0]) * int(self.env.block_size), int(intersection[1]) * int(self.env.block_size)), int(self.road_width/2))
